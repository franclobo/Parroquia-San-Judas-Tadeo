...
class Consulta < FXMainWindow
  def initialize(app)
    ...
    # seccioan consulta
    @groupbox = FXGroupBox.new(self, 'Elija una opción para su consulta',
                               opts: LAYOUT_EXPLICIT | FRAME_GROOVE | LAYOUT_FILL_X, width: 200, height: 120, x: 10, y: 100)
  ...
  def clear_input_fields
    ...
    return unless defined? @lbl_sacramento
  ...
  def create_fecha_fields
    ...
    @input_fecha = FXTextField.new(self, 10, opts: LAYOUT_EXPLICIT, width: 150, height: 20, x: 170,
                                             y: 340)
  end

  def create_sacramento_fields
    puts 'create_sacramento_fields'
    @lbl_sacramento = FXLabel.new(self, 'Sacramento', opts: LAYOUT_EXPLICIT, width: 150, height: 20, x: 10,
                                                      y: 340)
